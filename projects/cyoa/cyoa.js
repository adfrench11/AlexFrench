...
function renderPrompt(promptObj){
  ...
  $('#option-1').text(promptObj.option1.text);
  $('#option-1').val(promptObj.option1.value);
  $('#option-1').show();
  $('#option-2').text(promptObj.option2.text);
  $('#option-2').val(promptObj.option2.value);
  $('#option-2').show();
  if (promptObj.option3.text === "") {
    $('#option-3').hide();
  }
  else {
    $('#option-3').text(promptObj.option3.text);
    $('#option-3').val(promptObj.option3.value);
    $('#option-3').show();
  }
  $('#hidden-submit').hide();
...
function renderEnding(promptObj){
  $('#prompt').text(promptObj.ending);
  $('#option-1').text('Choose Another Adventure!');
  $('#option-2').hide();
  $('#option-3').hide();
  $('#hidden-submit').hide();
};
...
function respondToInput(){
  $('button').on('click', (event) => {
    ...
    if (targEl.innerHTML === 'Choose Another Adventure!') {
      renderAdventureContent(adventures.table);
    } else {
      let choice = targEl.value;
